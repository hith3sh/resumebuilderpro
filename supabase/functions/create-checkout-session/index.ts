...
serve(async (req) => {
  ...
  try {
    ...
    const { items, totalAmount, metadata = {}, isGuest = false } = await req.json()

    // Handle guest checkout
    if (isGuest) {
      console.log('Processing guest checkout - email will be collected by Stripe')
    } else {
    ...
    const sessionConfig = {
      ...
      metadata: {
        ...
        ...(isGuest ? {} : { userId: (user as any)?.id }),
        ...metadata,
      },
    }

    // For guest checkout, require email collection
    if (isGuest) {
      (sessionConfig as any).customer_creation = 'always'
